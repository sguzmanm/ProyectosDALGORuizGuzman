...
public class GraphExplorationCoinChangeAlgorithm implements CoinChangeAlgorithm {
	...
	private int boundNumberOfCoins;
	@Override
	public int[] calculateOptimalChange(int totalValue, int[] denominations) {
		...
		this.boundNumberOfCoins = new CoinChangeState(greedySol).getTotalCoins();
		//Call of the graph exploration algorithm to find feasible solutions
		List<CoinChangeState> solutions = findFeasibleSolutions();
		//Choose the best of the selected solutions
		CoinChangeState opt = null;
		int minCoins = 0;
		for(CoinChangeState sol:solutions) {
			int coins = sol.getTotalCoins();
			if(opt == null || minCoins > coins) {
				minCoins = coins;
				opt = sol;
			}
		}
	...
	private List<CoinChangeState> findFeasibleSolutions() {
		List<CoinChangeState> answer = new ArrayList<>();
		//Initial state
	 ...
	 */
	private boolean isSolution(CoinChangeState state) {
